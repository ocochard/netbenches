...
set title noenhanced "OpenVPN throughput: userland vs DCO (kernel data channel)\nPC Engines APU2 (4 cores AMD GX-412T and Gigabit Intel i210AT)"
set xlabel noenhanced "FreeBSD 16-CURRENT n313366 (BSDRP 2.3), OpenVPN 2.7.7, iflib.tx_abdicate=1\n5000 clear flows to encrypt, 500 Bytes UDP payload => 542B frame\nif_ovpn(4) supports no AES-CBC: no DCO bar for the two CBC cyphers\nMethodology for Benchmarking IPsec Gateways:\nhttp://www.mecs-press.org/ijcnis/ijcnis-v4-n9/IJCNIS-V4-N9-1.pdf"
...
set key on outside top center horizontal

# Ploting!
# Log scale: DCO is ~19x the userland null result, linear would flatten userland.
set logscale y
set yrange [10:1200]
set ytics format "%.0f"
plot "userland.data" using 2:3:4:xticlabels(1) with histogram title "userland" ls 2, \
     "dco.data" using 2:3:4 with histogram title "DCO (kernel if\\_ovpn)" ls 3
