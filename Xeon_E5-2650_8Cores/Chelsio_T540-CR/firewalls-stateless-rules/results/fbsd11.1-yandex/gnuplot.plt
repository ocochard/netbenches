...
set title noenhanced "Impact of stateless firewall configurations on FreeBSD 11.1 forwarding performance\n(harvest.mask=351, AFDATA and RADIX locks patched)\nHP ProLiant DL360p Gen8 with 8 cores Intel Xeon E5-2650 2.60GHz, Chelsio T540-CR"
set xlabel noenhanced "Number of (non-matched) denied rules before allow all"
...
# Ploting!
plot f(x) with lines title "Minimum throughput requiered" linestyle 4 lw 4, \
	"ipfw-stateless.data" using 2:3:4:xticlabels(1) with histogram title "ipfw" ls 2, \
	 "pf-stateless.data" using 2:3:4:xticlabels(1) with histogram title "pf (convert rules into one table!)" ls 3, \
	 "ipf-stateless.data" using 2:3:4:xticlabels(1) with histogram title "ipf" ls 1