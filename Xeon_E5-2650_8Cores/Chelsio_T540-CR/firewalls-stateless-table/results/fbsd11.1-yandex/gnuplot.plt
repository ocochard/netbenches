...
set title noenhanced "Impact of firewall table size on FreeBSD 11.1 forwarding performance\n(harvest.mask=351, AFDATA and RADIX locks patched)\nHP ProLiant DL360p Gen8 with 8 cores Intel Xeon E5-2650 2.60GHz, Chelsio T540-CR"
set xlabel noenhanced "Number of (non-matched) entries into table before allow all"
...
plot f(x) with lines title "Minimum throughput requiered" linestyle 4 lw 4, \
	"ipfw-table.data" using 2:3:4:xticlabels(1) with histogram title "ipfw" ls 2, \
	 "pf-table.data" using 2:3:4:xticlabels(1) with histogram title "pf" ls 3