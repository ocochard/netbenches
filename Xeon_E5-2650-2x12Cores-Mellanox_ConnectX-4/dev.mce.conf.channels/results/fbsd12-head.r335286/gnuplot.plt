...
set xlabel noenhanced "Number of queues (24 by default), HyperThreading and LRO/TSO disabled, harvest.mask=351"
...
plot e(x) with lines title "10Gigabit line-rate reference" linestyle 2 lw 4, \
	f(x) with lines title "linear reference" linestyle 3, \
	 "gnuplot.data" using 0:2:xtic(1) with lines title "FreeBSD 12-head r335286" ls 1, \
